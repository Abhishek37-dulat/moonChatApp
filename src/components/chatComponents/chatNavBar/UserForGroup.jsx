...
const UserForGroup = ({
  ...
  const handleChangeBox = () => {
    if (checkBoxNumber.includes(data?.id)) {
      let itemValue = checkBoxNumber.filter((item) => item !== data?.id);
      setCheckBoxNumber(itemValue);
    } else {
      let tempData = checkBoxNumber?.map((item) => item);
      setCheckBoxNumber([...tempData, data?.id]);
    }
  };
  ...
  return (
    <React.Fragment>
      <MainBox onClick={() => handleChangeBox()}>
        <Box
          style={{
        ...
        >
          <Checkbox
            checked={checkBoxNumber.includes(data?.id)}
